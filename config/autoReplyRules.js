...
// Edit this file to change what Auralivin's bot says. No need to touch other
// code. Rules are checked top to bottom; the first match wins.
...
module.exports = {
  ...
  rules: [
      ...
      // trackOrder tells webhook.js to expect the customer's very next
      // message to be their Order Number, and to look it up live in
      // Shopify instead of just replying with canned text.
      trackOrder: true,
      reply:
        'To check your order status, please reply with just your Order Number (e.g. 2586 or #2586) and I\'ll look it up for you.',
          },
          {
  ...
  // Sent when nothing above matches - CURRENTLY UNUSED. webhook.js now
  // stays silent instead of sending this, so the bot doesn't
  // auto-reply to general/random messages that aren't one of the
  // menu keywords. Kept here in case you want that behavior back -
  // see the comment above `return null;` near the end of
  // buildReply() in routes/webhook.js.
  fallback:
