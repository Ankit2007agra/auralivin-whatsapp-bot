...
// Tracks phone numbers currently being handled by the AI agent (after they
// picked "Talk to an Agent"), and each one's recent conversation turns so
// the AI has context. In-memory only, same caveat as above.
const aiAgentNumbers = new Set();
const aiConversationHistory = new Map();
const MAX_AI_HISTORY_TURNS = 12;

// Typing any of these while in AI-agent mode goes back to the numbered menu.
...
// A customer already being handled by the AI agent can type "menu" to
// go back to the numbered options instead of chatting with the AI.
if (aiAgentNumbers.has(from) && MENU_RESET_KEYWORDS.includes(lower)) {
      aiAgentNumbers.delete(from);
      aiConversationHistory.delete(from);
      return rules.greeting;
}

// Once a customer has picked "Talk to an Agent", every following message
// goes straight to the AI instead of the keyword rules below, so it can
// hold a natural conversation instead of re-matching "track"/"catalog"/etc.
if (aiAgentNumbers.has(from)) {
      return aiAgentReply(from, text);
}
...
for (const rule of rules.rules) {
      if (rule.match.some((keyword) => lower.includes(keyword))) {
            ...
            if (rule.aiAgent) {
                  aiAgentNumbers.add(from);
                  return aiAgentReply(from, text, rule.reply);
            }
            return rule.reply;
      }
}
...
// Gets a natural, AI-generated reply for a customer already routed to the
// AI agent (see lib/ai.js). Falls back to `fallbackText` (or a generic
// message) if ANTHROPIC_API_KEY isn't set or the AI call fails, so the
// customer always gets *something* back.
async function aiAgentReply(from, text, fallbackText) {
      const history = aiConversationHistory.get(from) || [];
      const reply = await ai.getAIReply(history, text);
...
const updated = [...history, { role: 'user', content: text }, { role: 'assistant', content: reply }].slice(
      ...
      aiConversationHistory.set(from, updated);
      return reply;
}
